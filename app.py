...
# Professional Web Interface Custom Layout Styling via CSS
st.markdown("""
    <style>
...
# ==========================================
# 2. DATA EXTRACTION LAYER (HOISTED TO TOP)
# ==========================================
...
# Pre-evaluate database execution states before drawing the sidebar
...
# ==========================================
# 3. SIDEBAR ACCESS MANAGEMENT (UI CONTROLS)
# ==========================================
st.sidebar.header("⚙️ Connection Configuration")

# MySQL UI Masking Block
if not db_verified:
...
with col1:
    ...
    # REALISTIC MATHEMATICAL PROFILE CALIBRATION
    st.markdown("---")
...
with col2:
    ...
    if not st.session_state["saved_pwd"]:
        ...
        # CONDITIONAL RENDER: DUAL-AXIS SUMMARY
        if df_salaries is not None and not df_salaries.empty:
        ...
        # HORIZONTAL COMPETENCY FREQUENCY GRAPH
...
# ==========================================
# 5. GLOBAL PIPELINE EXECUTION LAYER (OUTSIDE OF COLUMNS)
# ==========================================
st.markdown("---")

if db_verified:  # Only show action buttons if the database connectivity is active
                ...
                try:
                    ...
                    for attempt in range(max_retries):
                        try:
                            ...
                            # Catch and handle 503 Service Unavailable with backoff strategy
                    ...
                    if response:
                        ...
                        st.error(
                            "❌ Unable to reach the AI Engine after multiple retries. Please click the button to try again in a moment.")
